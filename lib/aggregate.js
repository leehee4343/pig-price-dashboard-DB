...
// 워크북 버퍼(또는 파일 경로)를 header:1 행 배열로 읽는다. fixSheetRange를 항상 거친다.
...
function readRowsFromFile(filePath) {
  return readRowsFromWorkbook(XLSX.readFile(filePath));
}

function readRowsFromBuffer(buffer) {
  return readRowsFromWorkbook(XLSX.read(buffer, { type: 'buffer' }));
}
...
function computeDashboardData(priceRows, companyRows) {
  const publicCompanyRows = [PUBLIC_COMPANY_FIELDS];
  const companyHeadersForPublic = companyRows[0] || [];
  for (let i = 1; i < companyRows.length; i++) {
    const row = companyRows[i] || [];
    publicCompanyRows.push(PUBLIC_COMPANY_FIELDS.map(field => row[companyHeadersForPublic.indexOf(field)] ?? ''));
  }
  ...
  const DATA = {
    ...
    data_quality: {
      ...
      "지급률결측_비율": validRows.length > 0 ? parseFloat((nullRateCount / validRows.length * 100).toFixed(1)) : 0,
  ...
  return { DATA, publicCompanyRows };
}

// index.html의 3개 데이터 홀더 스크립트 태그를 새 값으로 치환한 HTML 문자열을 반환한다.
// (파일을 직접 쓰지 않는다 — 호출자가 fs.writeFileSync든 GitHub API PUT이든 알아서 처리)
function injectIntoHtml(htmlContent, DATA, priceRows, publicCompanyRows) {
  ...
  if (!dataHolderRegex.test(htmlContent) || !rawPriceRegex.test(htmlContent) || !rawCompanyRegex.test(htmlContent)) {
    throw new Error('index.html 안에서 데이터 홀더 태그를 찾을 수 없습니다.');
  }

  let result = htmlContent;
  result = result.replace(dataHolderRegex, `<script id="DATA_HOLDER" type="application/json">${JSON.stringify(DATA)}</script>`);
  result = result.replace(rawPriceRegex, `<script id="RAW_PRICE_HOLDER" type="application/json">${JSON.stringify(priceRows)}</script>`);
  result = result.replace(rawCompanyRegex, `<script id="RAW_COMPANY_HOLDER" type="application/json">${JSON.stringify(publicCompanyRows)}</script>`);
  return result;
}
